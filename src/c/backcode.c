...
void poisson_solver_float(float *d, float *phi, float *phi_i, float *phi_ij, 
  ...
  else {printf("Do not smooth the input.\n"); fflush(stdout);}
  ...
  for (l=0; l<ngrid; l++)
	...
	//  }
          ...
          ArrayAccess3D_n3(dk, ngrid, ngrid, (ngrid/2+1), l, m, n)[0]=greens*dksig[0];
          ArrayAccess3D_n3(dk, ngrid, ngrid, (ngrid/2+1), l, m, n)[1]=greens*dksig[1];
	  }
        else {
          dksig[0]=W*ArrayAccess3D_n3(dk, ngrid, ngrid, (ngrid/2+1), l, m, n)[0];
          dksig[1]=W*ArrayAccess3D_n3(dk, ngrid, ngrid, (ngrid/2+1), l, m, n)[1];
  ...
  if((do_phi==TRUE)||(smooth_input==TRUE) ){
