...
def pk(d, dk=None, boxsize=500.,bin2fact=1./16., filename='',getnuminbin=False,overwrite=True,
       checkmean=False,getxi=False,binedges = None):
    ...
    if (os.path.isfile(filename))*(overwrite == False):
        ...
        if ((dk != None) & (d == None)):
            sk = dk.shape
            s = (dk.shape[0],dk.shape[0],dk.shape[0])
        else:
            if (checkmean):
            ...
            s = d.shape
            sk = dk.shape
        dk2 = (dk*N.conjugate(dk)).astype(N.float32).flatten()
        ...
        for i in N.arange(0,nbins-1):
            if (cuts[i+1] > cuts[i]):
                ...
                #print binedges[i],binedges[i+1],k[cuts[i]:cuts[i+1]]
...
def corfunk(d, d2=None, boxsize=500.,binsize=1, filename='',getnuminbin=False,overwrite=True,checkmean=False,exclude0lag=True,get2d=False,pisigma3d =False, getr=False):
    """
    d2 is the second field, for x-correlations
    """
    if (os.path.isfile(filename))*(overwrite == False):
        p = N.loadtxt(filename)
        kmean = p[:,0]
...
def corfunk_dens(d,slices=None,nslices=16,boxsize=None):
    ncells=len(d.flatten())
    if slices == None:
        sortd=N.sort(d.flatten())
        slices=sortd[N.arange(0,ncells,ncells/nslices)]
        slicemedians = sortd[N.arange(ncells/nslices/2,ncells,ncells/nslices)]

        return slices,slicemedians
    else:
        nslices = len(slices)
    print 'nslices=',nslices
    r,corall = corfunk(d,boxsize=boxsize)
    cordens = N.zeros((len(corall),nslices),dtype=N.float)
    for i in N.arange(nslices):
        print i,slices[i]
        if i == nslices-1:
            dslice = (d >= slices[i])
        else:
            dslice = (d >= slices[i]) & (d < slices[i+1])
        r,cordens[:,i] = corfunk(d,d2=dslice.astype(N.float),boxsize=boxsize)

    return r,corall,cordens
